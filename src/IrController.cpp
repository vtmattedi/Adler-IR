...
void AcIrStateController::nextState(IrCodes code)
{
    ...
    if (code == AC_POWER)
    {
        ...
        state.mode = AC_MODE_COOL; // reset to cool mode when toggling power
        return;
    }
    if (!state.power)
    {
    ...
    else if (code == AC_TURBO)
    {
        ...
        state.mode = AC_MODE_COOL; // turbo forces cool
    }
...
void AcIrStateController::setPower(bool powerState)
{
...
void AcIrStateController::setTemp(uint8_t temp)
{
    ...
    while (state.temp != temp && guard--)
    {
        if (!sendIRCode(state.temp < temp ? AC_PLUS : AC_MINUS))
            break; // queue full: stop rather than spin; the belief stays consistent with what was queued
    }
    if (!wasOn)
        sendIRCode(AC_POWER);
}

void AcIrStateController::setMode(AcIrMode mode)
{
