...
// ESP32_C3 is board specific, so it is set per env in platformio.ini rather than here.
...
// Required alongside COMPILE_TIMERS: Timers.h always defines BOOTSTRAP_TIMER_SYNC, and that
// timer publishes getSystemStatus(), which only exists when this is on.
